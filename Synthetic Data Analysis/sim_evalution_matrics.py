...
#PMI
def calculate_pmi(Phi,time_slice,N,corpus):
    '''
    Input: 
    Phi: topic-word distribution,K*V dimension
    ...
    for t in range(len(time_slice)):
        ...
        tmp_corpus = corpus[t*time_slice[0]:((t+1)*time_slice[0])]
...
def calculate_coherence(Phi, corpus_sample, num_topics,N_word,time_slice):
    """
    ...
    for t in range(T):
        ...
        corpus = corpus_sample[t*time_slice[0]:((t+1)*time_slice[0])]

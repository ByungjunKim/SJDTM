...
class SJDTM_para(utils.SaveLoad):
    def __init__(self,id2word=None,time_slice=None,topic_suffstats=None,num_topics=10,obs_variance=0.05,chain_variance=0.005,passes=10,random_state=None,chunksize=10):
        ...
        alphas:一个常数，主题分布的先验参数的一维形式
        num_topics:int，主题数量
        initialize:{'gensim','own','ldamodel'}
                'gensim':gensim's LDA
                'own':
                'ldamodel':之前训练的LDA模型
        sstats:时刻0的beta分布（主题词分布）
        lda_model:sufficient statistics来自的模型
        obs_variance:用于近似真实的向前方差
        chain_variance:beta的变分分布的方差参数
        passes:
        random_state:随机参数
        lda_inference_max_iter:LDA推断的最大迭代步数
        em_min_iter:EM算法的最小迭代步数
        em_max_iter:EM算法的最大迭代步数
        chunksize:文档的分块大小
        alpha_bar:alpha的平滑参数
        s:beta分布的先验参数
        v:beta分布的先验参数
        iterations_gamma_bt:gamma和bt部分更新的迭代次数
        ...
        # fit DTM
        self.fit_lda_seq_topics(topic_suffstats)#这个位置拟合
